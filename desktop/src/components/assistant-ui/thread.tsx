...
// ── Composer Extras Context (approval mode from ChatPanel) ───────────────────
export interface ComposerExtras {
  approvalMode: ApprovalMode | null;
  disabled: boolean;
  onSetApprovalMode: (mode: ApprovalMode) => void;
}
export const ComposerExtrasContext = createContext<ComposerExtras | null>(null);
...
const TOOL_CATEGORY_ICON: Record<string, ReactNode> = {
  terminal:   <TerminalIcon className="size-3 shrink-0" />,
  filesystem: <FileTextIcon className="size-3 shrink-0" />,
  browser:    <GlobeIcon    className="size-3 shrink-0" />,
  mcp:        <WrenchIcon   className="size-3 shrink-0" />,
  other:      <WrenchIcon   className="size-3 shrink-0" />,
};
...
const ToolStepRow: FC = () => {
  const message = useMessage();
  const custom = message.metadata?.custom as Record<string, unknown> | undefined;
  const toolLabel = (custom?._toolLabel as string | undefined) ?? (custom?._toolName as string | undefined) ?? "tool";
  const toolSummary = (custom?._toolSummary as string | undefined) ?? "";
  const toolKeyLine = (custom?._toolKeyLine as string | undefined) ?? "";
  const category = (custom?._toolCategory as string | undefined) ?? "other";
  const icon = TOOL_CATEGORY_ICON[category] ?? TOOL_CATEGORY_ICON.other;
  return (
    <div className="mx-auto w-full max-w-(--thread-max-width) px-2 py-1.5">
      {toolSummary && (
        <p className="mb-2 text-[13px] leading-6 text-foreground/90">
          {toolSummary}
        </p>
      )}
      <div className="inline-flex items-center gap-1.5 rounded-xl border border-border/60 bg-muted/40 px-3 py-1 text-[11px] text-muted-foreground">
        {icon}
        <span className="font-mono">{toolLabel}</span>
      </div>
      {!toolSummary && toolKeyLine && (
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent side="top" align="start" className="w-36">
            {(["default", "maximum"] as const).map((mode) => {
              const active = extras.approvalMode === mode;
              return (
                <DropdownMenuItem
                  key={mode}
    ...
    >
      <div className="aui-assistant-message-content wrap-break-word px-2 text-foreground leading-relaxed">
        <MessagePrimitive.Parts>
          {({ part }: { part: { type: string; toolUI?: React.ReactNode; [key: string]: unknown } }) => {
            if (part.type === "text") return <MarkdownText />;
            if (part.type === "tool-call")
              return (part.toolUI as React.ReactNode) ?? <ToolFallback {...(part as Parameters<typeof ToolFallback>[0])} />;
            return null;
          }}
        </MessagePrimitive.Parts>
        <MessageError />
